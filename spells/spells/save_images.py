...
with open('spells.json', 'r') as spells_file:
    ...
    for spell in spells_data:
        ...
        #     size_dict["left"] = int(left)
        #     size_dict["top"] = int(top)
        #     size_dict["right"] = int(right)
        #     size_dict["bottom"] = int(bottom)
        #     size_list.append(size_dict)
        # left = max(size_list, key=lambda x:x['left'])
        # top = max(size_list, key=lambda x:x['top'])
        # right = max(size_list, key=lambda x:x['right'])
        # bottom = max(size_list, key=lambda x:x['bottom'])
        x = 30
        for i in range(1, tags_len):
            position = (x, 80)
            left, top, right, bottom = I1.textbbox(position, spell['tags'][i].upper(), font=tags_font)
        ...
        #Tradition
        if spell['tradition']:
            I1.text((30, 120), "Tradições {}".format(spell['tradition']), font=stats_font, fill =(0, 0, 0))
        else:
            pass
        ...
        #     img.paste(action, (10, y_text+20), action)
